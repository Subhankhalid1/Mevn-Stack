<template>
    <section class="h-100">
      <div class="container-fluid h-100 py-5" style="width: 76%">
        <div
          ...
          v-if="allfavProducts.length"
        >
          <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
              <h3 class="fw-normal mb-0 text-black">Favourite List</h3>
              <div>
                <p class="mb-0">
                  <span class="text-muted">{{ allfavProducts?.length }}</span>
            ...
            >
              <div
                ...
                v-for="(cart, ind) in allfavProducts"
                :key="ind"
              >
  ...
  export default {
    computed: { ...mapGetters(["allfavProducts", "u_token"]) },
    created() {
      if (this.u_token) {
        this.getFavData();
      }
